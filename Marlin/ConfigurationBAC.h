...
// #define HIGH_ACCELERATION   // 是否提高加速度
...
#define USE_X_LEVELING      // 加入x轴调平
#define UPDATE_BOOT_FROM_SCREEN 0 // 1:Update boot from screen 0: Update boot from startup
...
#define HEATER_0_MAXTEMP2 315
#define HEATER_1_MAXTEMP2 315
#define HEATER_2_MAXTEMP2 315
#define HEATER_3_MAXTEMP2 315
#define HEATER_4_MAXTEMP2 315
#define HEATER_5_MAXTEMP2 315
#define HEATER_6_MAXTEMP2 315
#define HEATER_7_MAXTEMP2 315
 ...
 */
#if ENABLED(QUICK_PRINT)
  #define DEFAULT_AXIS_STEPS_PER_UNIT   { 106.7, 80, 106.7, 400 }
#elif ENABLED(TEST_FW)
  #define DEFAULT_AXIS_STEPS_PER_UNIT   { 800, 800, 800, 400 }
#elif ENABLED(MAGICIAN_X2)
  #define DEFAULT_AXIS_STEPS_PER_UNIT   { 80, 77.15, 409.5, 400 }
#else
...
#if ENABLED(QUICK_PRINT)
  #define DEFAULT_MAX_FEEDRATE          { 300, 300, 300, 90 }
#elif ENABLED(TEST_FW)
  #define DEFAULT_MAX_FEEDRATE          { 10, 10, 5, 45 }
#elif ENABLED(HIGH_ACCELERATION)
  #define DEFAULT_MAX_FEEDRATE          { 300, 300, 30, 90 }
#else
...
// #define LIMITED_MAX_FR_EDITING        // Limit edit via M203 or LCD to DEFAULT_MAX_FEEDRATE * 2
  ...
  #define MAX_FEEDRATE_EDIT_VALUES    { 600, 600, 10, 90 } // ...or, set your own edit limits
#endif
...
#elif ENABLED(HIGH_ACCELERATION)
  #define DEFAULT_MAX_ACCELERATION      { 2500, 2500, 100, 1000 }
#else
...
#else
#define Z_PROBE_SPEED_SLOW (8*60)  
#endif
...
#define DEBUG_LEVELING_FEATURE
  ...
  // 逐渐减少平台校正，直到达到设定的高度，在这里点的移动将与机器的XY平面持平。
  // 高度可以用 M420 Z<height> 来设置
  ...
  // 在Cartesian机器上，会像Delta那样将动作分割成一小段一小段而不是在网格边界上划分移动。
  // 在此之前的床层的轮廓比边对边的直线移动更接近。
...
#define LEVEL_BED_CORNERS
  ...
  #define LEVEL_CORNERS_INSET_LFRB { 30, 30, 30, 30 } // (mm) Left, Front, Right, Back insets // 调平时左右前后预留值
...
// Validate that endstops are triggered on homing moves // 验证在归巢移动时触发了端点
...
// 当启用时，当马林不能接受命令时，它将每隔几秒钟向主机发送一条忙碌状态消息。串口屏专用
//
...
// 此选项将覆盖生成一个步骤所需的默认编码器脉冲数。对于高分辨率编码器应增加。
//
...
// 使用此选项可以覆盖在下一个/上一个菜单项之间移动所需的步骤信号的数量。
//
...
// 风机和加热器采用软件PWM驱动。这使用了一个非常低的频率，这并不像硬件PWM那样令人讨厌。
// 另一方面，如果这个频率太低，您还应该增加SOFT_PWM_SCALE。
// #define FAN_SOFT_PWM
...
// 将此值增加1将使软件PWM频率增加一倍，影响加热器和风扇(如果FAN_SOFT_PWM启用)。
// 然而，控制分辨率将为每个增量减半;在0值处，有128个有效的控制位置。
// :[0,1,2,3,4,5,6,7]
...
//当SOFT_PWM_SCALE值大于0时，可以使用抖动来缓解分辨率损失。如果启用，一些PWM周期被拉长，因此平均期望的占空比是达到的。
